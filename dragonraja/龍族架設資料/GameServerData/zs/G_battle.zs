...
if( yes==1 ){
	DualCheck(1)
	if(yes == 1){
	LogDualQuest(72,Qcount,"듀얼 1차 퀘스트",0)	;;20030617 듀얼 퀘스트 로그 파일을 남기기 위해 개굴이가 추가
	Qcount = 0
	GhostLv = 0
	SendSound(0, 1311)
...
if( yes==1 ){
	D("임무는 총 6종류가 있으며 수시로 주어지는 임무가 바뀌게 됩니다.")
...
if( yes==1 ){
	...
	D("이번 임무는 헤게모니아의 무녀를 도와주는 일입니다.")	;Stay0
	SetQuestNumStep(29,0)
	D("그녀는 헤게모니아 42, 530에 서있습니다.")		;Stay108
	SetQuestNumStep(72,108)
	goto End_QuestNpc00032 
}
	if(ran>4){
	SendSound(0,64)
...
if( yes==1 ){
  ...
  LogDualQuest(72,Qcount,"듀얼 1차 퀘스트",1)	;;20030617 듀얼 퀘스트 로그 파일을 남기기 위해 개굴이가 추가
	goto End_QuestNpc00032 
}
...
else	
{
	...
	if(ran==0) 
	{
	D("복권은 36레벨을 넘겨야만 구입할 수 있다네~")
	}
	if(ran==1) 
	{
	D("아직까지 36레벨도 못 되었나? 좀 더 노력해서 오게나.")
	}
	if(ran==2) 
	{
	D("이 복권으로 조성된 기금은 전쟁 고아에게 사용되지.")
	}	
}
end
@Npc00034	
